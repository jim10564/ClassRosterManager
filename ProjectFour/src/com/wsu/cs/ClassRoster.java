...
public class ClassRoster {
    ...
    public boolean searchBothLists(int ID){
        if (findStudentFromList(ID) || myWaitList.findStudentFromWaitList(ID) ){
            return true;
        }
        return false;
    }
    ...
    /**
     * Prints the student with the user entered Student ID
     *
    ...
    public void dropStudentFromList(int userID) {
        for (Student s: classRoster){
            if (s.getStudentID() == userID){
                System.out.println("Student with ID " + userID + " Has been dropped from the Class Roster");
            }
        }
        ...
        if (myWaitList.getWaitList().size()!=0 && classRoster.size() < 5){
            classRoster.add(myWaitList.getWaitList().poll());
        }
